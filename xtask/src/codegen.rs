use crate::{
    ...
    project_root, write_to_file, AST_NODES, AST_TOKENS, SYNTAX_KINDS,
};
...
pub(crate) fn generate_syntax() -> Result<()> {
    ...
    let content = generate_syntax_kinds(KINDS_SRC)?;
    write_to_file(content, &syntax_kinds_file)?;

    let ast_tokens_file = project_root().join(AST_TOKENS);
    let content = generate_tokens(AST_SRC)?;
    write_to_file(content, &ast_tokens_file)?;

    let ast_nodes_file = project_root().join(AST_NODES);
    let content = generate_nodes(AST_SRC)?;
    write_to_file(content, &ast_nodes_file)?;
...
fn generate_syntax_kinds(grammar: KindsSrc<'_>) -> Result<String> {
    ...
    let full_keywords = full_keywords_values.iter().map(|kw| format_ident!("{}_KW", to_upper_snake_case(kw)));

    let all_keywords_values = grammar.keywords.to_vec();
    let all_keywords_idents = all_keywords_values.iter().map(|kw| format_ident!("{}", kw));
    let all_keywords =
        all_keywords_values.iter().map(|name| format_ident!("{}_KW", to_upper_snake_case(name))).collect::<Vec<_>>();
...
fn generate_tokens(grammar: AstSrc<'_>) -> Result<String> {
    ...
    let pretty = crate::reformat(quote! {
        use crate::{SyntaxKind::{self, *}, SyntaxToken, AstToken};
        #(#tokens)*
    })?
    .replace("#[derive", "\n#[derive");
    Ok(pretty)
...
fn generate_nodes(grammar: AstSrc<'_>) -> Result<String> {
    ...
    let (enum_defs, enum_boilerplate_impls): (Vec<_>, Vec<_>) = grammar
        ...
        .map(|en| {
            ...
            let kinds: Vec<_> =
                variants.iter().map(|name| format_ident!("{}", to_upper_snake_case(&name.to_string()))).collect();
            let doc = en.doc;
    ...
    let display_impls = enum_names.chain(node_names.clone()).map(|it| format_ident!("{}", it)).map(|name| {
        quote! {
            impl std::fmt::Display for #name {
                fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                    std::fmt::Display::fmt(self.syntax(), f)
